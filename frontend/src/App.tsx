        ...
        const checkToken = async () => {
            ...
            if (token) {
                ...
                if (parsedToken.expiresAt <= currentTime) {
                    ...
                    await refreshAccessToken(parsedToken.refreshToken);
                }
                if (athleteInfo) {
    ...
    return (
        <div className="App" style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', maxWidth: '100vw', justifyContent: 'space-between' }}>
            ...
            <main style={{ paddingTop: '5px', paddingBottom: '5px', textAlign: 'center' }}>
                {/* TODO - Cleanup to avoid nested ternary operators */}
                {isLoading ? (<StravaLoading message="Loading your profile..." />)
                    : (athlete ? (
                        <div>
                            <button
...
const fetchAccessToken = async (code: string) => {
    try {
        ...
        if (data.athlete) {
            ...
            console.log('Athlete info stored in local storage.');
        }
...
const refreshAccessToken = async (refreshToken: string) => {
    try {
        ...
        if (data.accessToken) {
            ...
            localStorage.setItem('token', JSON.stringify(tokenInfo)); // Store temporary access token in local storage
            console.log('Access token refreshed and stored in local storage.');
        } else {
            console.error('Access token not found in refresh token response.');
        }
    } catch (error) {
...
const StravaLoading: React.FC<{ message?: string }> = ({ message = 'Authorizing with Strava...' }) => (
    <>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <p style={{ marginTop: '20px', fontSize: '1.2rem', color: '#333' }}>{message}</p>
        </div>
        <style>{`
            @keyframes spin {
